...
import { IUserNomarca } from 'src/app/shared/models/interfaces/Table.interface';
...
export class UserTableComponent implements AfterViewInit, OnInit {
  ...
  dataSource = new MatTableDataSource<IUserNomarca>();
  ...
  getRemoteData(): void {
    this.NomarcaService.getAllUser().subscribe((data: any) => {
  ...
  createFilter() {
    let filterFunction = function (
      ...
      let nameSearch = () => {
        ...
        return (
          data.usuario.toString().toLowerCase().indexOf(searchTerms.usuario) !==
            -1 &&
          ...
          data.destinado
            .toString()
            .toLowerCase()
            .indexOf(searchTerms.destinado) !== -1 &&
          data.telefono
            .toString()
            .toLowerCase()
            .indexOf(searchTerms.telefono) !== -1 &&
          data.cgasto?.toString().toLowerCase().indexOf(searchTerms.cgasto) !==
            -1 && 
            data.categoria?.toString().toLowerCase().indexOf(searchTerms.categoria) !==
            -1 &&
  ...
  deleteUser(row: any) {
    ...
    let data: IConfirmDialogData = {
      type: 'confirm',
      title: '¿Estas seguro de que quieres eliminar el ususario?',
    };

    this.confirmService.show(data).subscribe((result) => {
      if (result) {
        this.NomarcaService.deleteUser(id).subscribe(
          () => {
            this.ShowAlertService.showCustomAlert(
              'usuario borrado: ' + row.nombre,
              'success',
              5,
              'top',
              'center'
          ...
          (error) => {
            this.ShowAlertService.showCustomAlert(
              'Error al borrar el usuario: ' + row.nombre,
              'error',
              5,
              'top',
              'center'
